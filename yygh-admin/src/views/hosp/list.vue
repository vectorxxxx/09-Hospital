  ...
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column label="医院logo">
        <template slot-scope="scope">
          <img :src="'data:img/jpeg;base64,'+ scope.row.logoData" width="80">
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    fetchData(page = 1) {
      this.page = page
      hosApi.getPageList(this.page, this.limit, this.searchObj).then(response => {
    ...
    // 查询所有省
    findAllProvince() {
      hosApi.findByDictCode('Province').then(response => {
    ...
    provinceChanged() {
      ...
      hosApi.findChildId(this.searchObj.provinceCode).then(response => {
    ...
    updateStatus(id, status) {
      hosApi.updateStatus(id, status).then(response => {
