            ...
            <div
              ...
              <div class="sub-item-wrapper">
                <div
                  ...
                  :key="it.id"
                  class="sub-item"
                  @click="schedule(it.depcode)"><span class="v-link clickable">{{ it.depname }} </span></div>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    // 初始化
    init() {
      hosApi.show(this.hoscode).then(response => {
        this.hospital = response.data.hospital
        this.bookingRule = response.data.bookingRule
      })
      hosApi.findDepartment(this.hoscode).then(response => {
